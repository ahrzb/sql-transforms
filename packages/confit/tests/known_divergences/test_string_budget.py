...
# DuckDB's lpad and rpad take INTEGER, and its binder does NOT implicitly
# downcast: a BIGINT count -- a row column, or even 2::BIGINT -- is a binder
# error there. The count binds only when it is spelled a way DuckDB
...
# The count check runs BEFORE the NULL short-circuit: otherwise a bare-NULL
# string lets a BIGINT count slip through -- lpad(NULL, c1, 'x') would serve
# NULL where DuckDB still binder-errors on the count. A NULL
...
# A literal pad/repeat count that can exceed the engine's 1 GiB string-builder budget
# refuses at build by name. Data-driven counts (a column, CAST(k AS INTEGER))
# keep the documented runtime cap.
#
# THE GROUND. The refusal is not forced by DuckDB instability. Measured,
# DuckDB is entirely deterministic:
#
