...
use crate::datafusion::expr::{Expr, Value};
use crate::datafusion::types::Schema;

// Re-exported so the engine-internal paths (`crate::datafusion::plan::InterpError`)
// keep resolving now that the error type is shared.
pub use crate::error::InterpError;
...
pub fn build_plan(sql: &str) -> Result<Plan, InterpError> {
    ...
    if let Some(predicate) = &select.selection {
        input = RelNode::Filter {
            input: Box::new(input),
            predicate: crate::datafusion::expr_build::convert_expr(predicate)?,
        };
    }
...
fn extract_equality_keys(expr: &SqlExpr) -> Result<Vec<(Expr, Expr)>, InterpError> {
    match expr {
        ...
        SqlExpr::BinaryOp {
            ...
            crate::datafusion::expr_build::convert_expr(left)?,
            crate::datafusion::expr_build::convert_expr(right)?,
        )]),
        _ => Err(InterpError::Build(
    ...
    for item in items {
        match item {
            SelectItem::UnnamedExpr(e) => {
                ...
                out.push((name, crate::datafusion::expr_build::convert_expr(e)?));
            }
            SelectItem::ExprWithAlias { expr, alias } => {
                // The output alias folds like any identifier (`AS Foo` -> `foo`).
                out.push((
                    crate::datafusion::expr_build::fold_ident(alias),
                    crate::datafusion::expr_build::convert_expr(expr)?,
                ));
            }
...
fn column_name(e: &SqlExpr) -> Result<String, InterpError> {
    match e {
        SqlExpr::Identifier(ident) => Ok(crate::datafusion::expr_build::fold_ident(ident)),
        SqlExpr::CompoundIdentifier(parts) => Ok(parts
            .last()
            .map(crate::datafusion::expr_build::fold_ident)
            .unwrap_or_default()),
...
pub fn execute(
    ...
    for row in &rows {
        ...
        for (alias, e) in &plan.projection {
            result.insert(alias.clone(), crate::datafusion::expr::eval(e, row)?);
        }
        out.push(result);
    }
    Ok(out)
...
fn execute_rel(
    ...
    match node {
        ...
        RelNode::Filter { input, predicate } => {
            ...
            let mut out = Vec::new();
            for row in rows {
                if let Value::Bool(true) = crate::datafusion::expr::eval(predicate, &row)? {
                    out.push(row);
                }
            }
            Ok(out)
        ...
        RelNode::Join {
            ...
            for l in &left_rows {
                for r in &right_rows {
                    ...
                    for (le, re) in on {
                        let lv = crate::datafusion::expr::eval(le, &merged)?;
                        let rv = crate::datafusion::expr::eval(re, &merged)?;
        ...
        RelNode::LookupJoin {
            ...
            for mut row in rows {
                let key: Vec<Value> = keys
                    .iter()
                    .map(|k| crate::datafusion::expr::eval(k, &row))
                    .collect::<Result<_, _>>()?;
        ...
        RelNode::Unnest {
            ...
            let mut out = Vec::new();
            for row in rows {
                match crate::datafusion::expr::eval(list_expr, &row)? {
                    Value::List(items) => {
                    ...
                    other => {
                        return Err(InterpError::Eval(format!(
                            ...
                            crate::datafusion::expr::type_name(&other)
                        )))
                    }
                }
            }
...
fn unnest_list_element(
    e: &Expr,
    effective_schemas: &HashMap<String, Schema>,
) -> Result<Option<(Expr, crate::datafusion::types::FieldType)>, InterpError> {
    ...
    let arg_ty = crate::datafusion::types::infer_type(&args[0], effective_schemas)?;
    let crate::datafusion::types::Base::List(elem) = arg_ty.base else {
        return Ok(None);
    };
...
fn expand_unnest_struct(
    ...
    let arg = &args[0];
    let arg_ty = crate::datafusion::types::infer_type(arg, effective_schemas)?;
    let crate::datafusion::types::Base::Struct(fields) = &arg_ty.base else {
        return Ok(None);
    };
...
fn validate_expr(
    ...
    match e {
        ...
        Expr::FieldAccess { base, field } => {
            ...
            let base_ty = crate::datafusion::types::infer_type(base, effective_schemas)?;
            match &base_ty.base {
                crate::datafusion::types::Base::Struct(fields) => {
