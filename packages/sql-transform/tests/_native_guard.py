...
# This crate links confit for the shared error/type/schema vocabulary, so a
# change there makes _interpreter stale too.
_LINKED_SRC = _REPO.parent / "confit" / "src"


def _newest_rs_mtime(src_dir: Path) -> float:
    dirs = [src_dir, _LINKED_SRC] if src_dir == _SRC else [src_dir]
    return max(
        (p.stat().st_mtime for d in dirs for p in d.glob("**/*.rs")),
        default=0.0,
    )
