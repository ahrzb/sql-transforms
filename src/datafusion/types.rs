//! DataFusion-flavoured static type inference over a projection expression.

use std::collections::{HashMap, HashSet};

// Re-exported so the engine-internal paths (`crate::datafusion::types::Base`)
// keep resolving now that the type vocabulary is shared.
pub use crate::types::{Base, FieldType, Schema};

use crate::datafusion::expr::{BinOp, CastType, Expr, Value};
use crate::datafusion::plan::InterpError;

/// Statically infers the FieldType of a projection expression, mirroring
/// `crate::datafusion::expr::eval()`'s structure but computing a type instead of a
