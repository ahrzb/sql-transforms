//! The DataFusion-semantics interpreter: `InferFn`.
//!
//! DataFusion is the oracle this engine is differentially tested against, so
//! every semantic choice here (identifier folding, NULL propagation, numeric
//! coercion, float ordering) mirrors DataFusion rather than standard SQL.
...
use crate::lookup::{self, LookupIndex};
use crate::schema;
...
fn validate_output_model(
    ...
    for (alias, expr) in projection {
        ...
        if !crate::types::compatible(&inferred.base, &declared.base) {
