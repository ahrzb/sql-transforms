...
use crate::datafusion::types::FieldType;

// Re-exported so the engine-internal paths (`crate::datafusion::expr::Value`)
    ...
    match expr {
        ...
        Expr::Struct(fields) => {
            let values = fields
                ...
                .collect::<Result<_, crate::datafusion::plan::InterpError>>()?;
        ...
        Expr::Transform {
            ...
            let fields = match eval(arg, row)? {
                ...
                other => {
                    return Err(crate::datafusion::plan::InterpError::Eval(format!(
...
fn concat_op(l: Value, r: Value) -> Result<Value, crate::datafusion::plan::InterpError> {
    ...
    match (l, r) {
        (Value::Int(a), Value::Int(b)) => {
            if matches!(op, BinOp::Div | BinOp::Mod) && b == 0 {
                return Err(crate::datafusion::plan::InterpError::Eval(
...
fn as_f64(v: &Value) -> Result<f64, crate::datafusion::plan::InterpError> {
    match v {
...
fn logic(op: BinOp, l: Value, r: Value) -> Result<Value, crate::datafusion::plan::InterpError> {
    let lb = as_tribool(&l)?;
    let rb = as_tribool(&r)?;
    Ok(match op {
...
fn as_tribool(v: &Value) -> Result<Option<bool>, crate::datafusion::plan::InterpError> {
    match v {
...
fn as_str(args: &[Value], idx: usize) -> Result<&str, crate::datafusion::plan::InterpError> {
    match args.get(idx) {
...
fn as_i64(args: &[Value], idx: usize) -> Result<i64, crate::datafusion::plan::InterpError> {
    match args.get(idx) {
...
fn eval_cast(v: Value, target: CastType) -> Result<Value, crate::datafusion::plan::InterpError> {
    use crate::datafusion::plan::InterpError;
