...
One class per tag, carrying **every** field DuckDB emits — including the ones
nothing reads, because a field that is not carried is a field that is dropped,
and a dropped field is different SQL.
...
type Node = Select | SetOperation | BaseTable | SubqueryRef | SubqueryExpr \
          | Join | ColumnRef | Function | WindowAggregate | ... | Opaque
```
...
`scripts/gen_ast_nodes.py`, run by hand. Nothing is generated at import.

```python
for sql in CORPUS:
    walk(_serialize(sql))        # tag -> field -> value type, nullability, nestedness

emit("packages/sql-transform/sql_transform/model/_nodes.py")   # the 53 classes
emit("packages/sql-transform/sql_transform/model/_shapes.json") # what they came from
```

Both artifacts are committed and reviewed like ordinary code.

Generated rather than hand-written for one reason: 498 field declarations
transcribed by eye, where **a single missed field is accepted and changes the
query**. The generator reads the fields off the oracle, so the lists are right
by construction instead of by proofreading.

It reports its own coverage (`53 tags from 719 statements`) rather than
implying completeness. There is no way to enumerate DuckDB's serialization
vocabulary from Python; everything unobserved lands in `Opaque`, and the number
is printed so nobody has to guess how wide the net was.

### The bump story

```bash
uv run python scripts/gen_ast_nodes.py
git diff        # this diff IS the drift report
```

`_shapes.json` pins `{duckdb: "1.5.5", shapes: {...}}`. A test re-derives the
shapes from the corpus and diffs against the manifest, so upgrading DuckDB
fails as
...
rather than as a wrong answer three layers down. Regenerating is the reviewable
fix.

The manifest catches drift in fields **nothing reads yet**, which parse-time
validation cannot: a field only validates when something asks for it.

## Staging

1. **The file.** Generator, `_nodes.py`, `_shapes.json`, the lossless gate and
   the manifest gate. Lands alongside; `_ast.py` and the walk are untouched.
   Nothing in the model's behaviour changes.
...
- **It does not make the model exhaustive over DuckDB.** It is exhaustive over
  what 719 statements exercise, and honest about the rest via `Opaque` plus a
  printed coverage number.
...
- **It does not pin the DuckDB version.** A bump is a regenerate-and-review,
  not a blocked upgrade — the manifest reports what moved and the suite says
  whether it mattered.
