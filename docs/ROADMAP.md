...
Active — in order:
1. [ ] [Rich (recursive) type system and UNNEST](BACKLOG.md#rich-recursive-type-system-and-unnest) — **foundation (2026-07-16):** replace the closed scalar type layer with a recursive, schema-driven one — struct + list container types + `UNNEST` (DataFusion has no `struct.*`) + dynamic Python↔`Value` marshalling. Supersedes the earlier "Rust struct-support" ticket. Blocks #2; also M4 feature-contract groundwork. Hard piece: `unnest(list)`'s row-cardinality change (new relational operator).
2. [ ] [Recursive (fit-cascade) composition — unfitted `{a}(col)`](BACKLOG.md#compose-sqltransforms-via-transformcol-references--follow-up-slices) — **design settled, parked pending #1** ([spec](superpowers/specs/2026-07-16-fit-cascade-composition-design.md)); output model becomes struct + `UNNEST` on the new type layer. Staged fit (fit inner → transform training forward → fit outer), learned state into the *composite*. The primitive sklearn `Pipeline` composition builds on. (Fan-out + multi-input still deferred — they re-enter with the sklearn transformers that need them, e.g. OneHot fan-out.)
