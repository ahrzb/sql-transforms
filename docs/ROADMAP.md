...
Active — in order:
1. [ ] [Recursive (fit-cascade) composition — unfitted `{a}(col)`](BACKLOG.md#compose-sqltransforms-via-transformcol-references--follow-up-slices) — **unblocked** (its struct + `UNNEST` output-model dependency is satisfied by the rich-type slice); design settled, still parked pending a go-ahead to start ([spec](superpowers/specs/2026-07-16-fit-cascade-composition-design.md)). Staged fit (fit inner → transform training forward → fit outer), learned state into the *composite*; output model = struct + `UNNEST`. The primitive sklearn `Pipeline` composition builds on. (Fan-out + multi-input still deferred — they re-enter with the sklearn transformers that need them, e.g. OneHot fan-out.)
