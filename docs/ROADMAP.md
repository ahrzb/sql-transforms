...
- **`backlog/docs` · `backlog/decisions` · `backlog/drafts`** — the reasoning archive:
  design briefs, ADRs (the *why*), and parked/not-yet-scoped work (was `BACKLOG.md`).
...
Done — foundation:
- [x] [First slice: frozen composition `{a.transform}(col)`](<../backlog/docs/doc-8 - Composition-—-transformcol-references-mechanics-and-deferred-slices.md>) — shipped on master; the frozen-reuse primitive the recursive path extends.
...
1. [ ] [sklearn integration — functionality & parity](<../backlog/docs/doc-2 - sklearn-transformer-implementation-plan.md>) — **fallback-first (decided 2026-07-16)**, built on the recursive composition now shipped. The **fallback phase** (below) stands up the compose-in surface with real-sklearn-fallback internals; the **native-swap phase** then swaps each transformer to a native engine implementation, one at a time, diffed against the fallback oracle, in **transformer-tier order** (see [the sklearn transformer plan](<../backlog/docs/doc-2 - sklearn-transformer-implementation-plan.md>): Tier 0 `StandardScaler`/`SimpleImputer`/`OrdinalEncoder`/`OneHotEncoder` → Tier 1 scalers + `TargetEncoder` → Tier 2). Most Tier 0/1 map onto already-shipped engine machinery (window aggs, `PARTITION BY`, `LookupJoin`, struct/`UNNEST`). n = 1 serving-path speed is the separate serving-path phase.
   ...
   > DRAFT-1, estimator-interface compliance). Dev is designing the
   ...
   > shipped** (see decision-3, opaque-transform split). **Part 1 —
   ...
   > Both carry small review follow-ups (**TASK-2** Part 1 / **TASK-3** Part 2). **Next:**
   > the native-swap phase (native-per-transformer) + the `ColumnTransformer` assembly surface.

   **Fallback-phase slices (superseded — pending fallback-node design):**
   1. [ ] **Thin-vertical slice.** Estimator interface (`fit`/`transform`/`get_feature_names_out`/`get_params`/`set_params`/cloneable) on ONE transformer (`StandardScaler`), internals = real sklearn fallback, driven end-to-end through a stock sklearn `Pipeline`. **Stands up the [per-transformer differential parity harness](<../backlog/docs/doc-2 - sklearn-transformer-implementation-plan.md>)** (StandardScaler through the param + edge-case matrix). Ships hooks 1+3 on its own. Designs the `get_feature_names_out`/provenance **contract shape** (the hook-3 surface) — joint look before it hardens. NB: this slice is single-transformer parity; the full *assembly* oracle lands in the ColumnTransformer-glue slice.
...
- [ ] [Transformer execution model — UDF/UDAF, macros](<../backlog/docs/doc-7 - Transformer-execution-model-—-UDF-UDAF-macros-composition.md>) — the "conceptual backbone," but not immediately useful; spec to be *extracted from* the concrete work above rather than written up front.
...
- [ ] Benchmark inference-path optimizations before building them — see **DRAFT-4**
...
- [ ] Rust-optimized serving inference path — see **DRAFT-3**
...
- [ ] Typed / validated / provenance feature contract — *(no ticket yet; scope once the transformer-foundation phase lands)*

## Later / parked
- [ ] `CASE WHEN` in authored SQL — ticketed (**TASK-27** native / **TASK-30** codegen); outer joins parked (**DRAFT-6**)
- [ ] `ORDER BY` / window frames — research spike (**DRAFT-5**); may not fit a row-at-a-time inference engine
