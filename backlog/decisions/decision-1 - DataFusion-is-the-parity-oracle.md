...
`infer`/`infer_batch` (the Rust `InferFn` interpreter, row-at-a-time). The README
...
harness gates on it. Where `infer` (Rust) disagrees with `transform`, **Rust is
wrong** — match DataFusion's behavior; never codify current Rust behavior as
correct. This holds for values on the normal numeric path (error *types* are a
separate non-goal — see decision-2).

## Consequences

- Every parity bug is defined as "Rust diverges from DataFusion," and the fix
